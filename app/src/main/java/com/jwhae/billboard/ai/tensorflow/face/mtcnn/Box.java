...
public class Box {
    ...
    public Box(RectF rect){
        ...
        box[0] = (int)rect.left;
        box[1] = (int)rect.top;
        box[2] = (int)rect.right;
        box[3] = (int)rect.bottom;
    }
    ...
    public Point center(){return new Point(left() + width() / 2, top() + height() / 2);}
    //转为rect
    ...
    public List<PointF> return2PointF(){
        ArrayList<PointF> pointArray = new ArrayList<>();
        for(int i = 0; i < landmark.length ; i++){
            pointArray.add(new PointF(landmark[i]));
        }
        return pointArray;
    }
